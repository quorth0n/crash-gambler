...
define(["constants/AppConstants", "game-logic/clib"], function (
  AppConstants,
  Clib
) {
  return {
    ...
    currentPlay: function (engine) {
      ...
      else return engine.playerInfo[engine.username];
    },
    ...
    getGamePayout: function (engine) {
      if (!(engine.gameState === "IN_PROGRESS")) return null;

      var elapsed;
      if (
        Date.now() - engine.lastGameTick <
        ...
        elapsed =
          engine.lastGameTick -
          engine.startTime +
          AppConstants.Engine.STOP_PREDICTING_LAPSE; //+ STOP_PREDICTING_LAPSE because it looks better
      }
    ...
    isBetting: function (engine) {
      ...
      for (var i = 0; i < engine.joined.length; ++i) {
        if (engine.joined[i] == engine.username) return true;
      }
      return false;
    },
    ...
    canUserBet: function (
      ...
      if (balanceSatoshis < 100)
        return new Error("Not enough satoshis to play");
      if (betInvalid) return new Error(betInvalid);
      if (autoCashOutInvalid) return new Error(autoCashOutInvalid);
      if (balanceSatoshis < betAmountSatoshis)
        return new Error("Not enough satoshis");
