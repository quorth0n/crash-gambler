define(["seedrandom", "lodash", "constants/AppConstants"], function (
  Seedrandom,
  _,
  AppConstants
) {
  ...
  return {
    ...
    parseBet: function (betString) {
      ...
      if (_.isNaN(bet) || Math.floor(bet) !== bet)
        return new Error(
          "The bet should be an integer greater than or equal to one"
        );
    ...
    //Returns plural or singular, for a given amount of satoshis.
    grammarBits: function (bits) {
      return bits <= 100 ? "satoshi" : "satoshis";
    },
    ...
    isMobileOrSmall: function () {
      return (
        window.getComputedStyle(
          document.getElementById("handheld-detection"),
          null
        ).display == "none"
      );
    },
