...
define([
  ...
  Graph.prototype.drawGraph = function () {
    ...
    for (var t = 0, i = 0; t <= this.currentTime; t += 100, i++) {
      ...
      this.ctx.lineTo(x + this.xStart, y); //TODO: Avoid doing the code above this if it will do this

      /* Draw green line if last game won */ /*var realPayout = Clib.payout(this.betSize, t);
  ...
  Graph.prototype.drawGameData = function () {
    ...
    if (Engine.gameState === "ENDED") {
      ...
      this.ctx.fillText(
        "Busted",
        this.canvasWidth / 2,
        this.canvasHeight / 2 - fontSizeNum(15) / 2
      );
      this.ctx.fillText(
        "@ " +
          Clib.formatDecimals(Engine.tableHistory[0].game_crash / 100, 2) +
          "x",
        this.canvasWidth / 2,
        this.canvasHeight / 2 + fontSizeNum(15) / 2
      );
