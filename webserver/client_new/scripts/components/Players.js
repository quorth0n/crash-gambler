define([
  ...
  "game-logic/engine",
  "classnames",
], function (React, Clib, _, Engine, CX) {
  var D = React.DOM;
    ...
    render: function () {
      ...
      if (game.gameState === "STARTING") {
        ...
        _.forEach(game.playerInfo, function (player, username) {
          if (player.stopped_at) usersWonCashed.push(player);
      ...
      if (game.gameState === "IN_PROGRESS" || game.gameState === "STARTING") {
        var i, length;
        var bonusClass =
          game.gameState === "IN_PROGRESS" ? "bonus-projection" : "";
        ...
        for (i = 0, length = usersLostPlaying.length; i < length; i++) {
          ...
          var bonus =
            game.gameState === "IN_PROGRESS"
              ? user.bonus
                ? Clib.formatDecimals((user.bonus * 100) / user.bet, 2) + "%"
                : "0%"
              : "-";
          var classes = CX({
            "user-playing": true,
          ...
          trUsersLostPlaying.push(
            D.tr(
              ...
              D.td(null, "-"),
              D.td(null, user.bet ? Clib.formatSatoshis(user.bet, 0) : "?"),
