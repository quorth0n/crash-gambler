define([
  ...
  "game-logic/engine",
  "game-logic/stateLib",
], function (React, _, Clib, Engine, StateLib) {
  var D = React.DOM;
  ...
  return React.createClass({
    ...
    render: function () {
      ...
      if (this.state.engine.gameState === "ENDED") {
        ...
        mePlayingClass = StateLib.currentlyPlaying(this.state.engine)
          ? "bet-bar-me-lost"
          : "bet-bar-me-won";
      } else {
      ...
      return D.div(
        { className: "bet-bar-container" },
        D.div({
          className: cashedWonClass,
          style: { width: betPercentages.cashedWon + "%" },
        }),
        D.div({
          className: mePlayingClass,
          style: { width: betPercentages.me + "%" },
        }),
        D.div({
          className: cashedWonClass,
          style: { width: betPercentages.cashedWonAfter + "%" },
        }),
        D.div({
          className: playingLostClass,
          style: { width: betPercentages.playingLost + "%" },
        })
      );
    },
  });
  ...
  function calculatePlayingPercentages(engine) {
    ...
    var myStop =
      currentPlay && currentPlay.stopped_at ? currentPlay.stopped_at : 0;

    _.forEach(engine.playerInfo, function (player, username) {
      if (username !== engine.username)
        if (player.stopped_at) {
          if (player.stopped_at > myStop) bitsCashedOutAfterMe += player.bet;
          else bitsCashedOut += player.bet;
        } else {
          bitsPlaying += player.bet;
        }
    });

    var totalAmountPlaying =
