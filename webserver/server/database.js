...
exports.expireSessionsByUserId = function (userId, callback) {
  ...
  query(
    "UPDATE sessions SET expired = now() WHERE user_id = $1 AND expired > now()",
    [userId],
    callback
  );
};
...
function createSession(
  ...
  if (remember) expired.setFullYear(expired.getFullYear() + 10);
  else expired.setDate(expired.getDate() + 21);

  client.query(
    "INSERT INTO sessions(id, user_id, ip_address, user_agent, expired) VALUES($1, $2, $3, $4, $5) RETURNING id",
...
exports.createOneTimeToken = function (userId, ipAddress, userAgent, callback) {
  ...
  query(
    "INSERT INTO sessions(id, user_id, ip_address, user_agent, ott) VALUES($1, $2, $3, $4, true) RETURNING id",
...
exports.getUserFromUsername = function (username, callback) {
  ...
  query(
    "SELECT * FROM users_view WHERE lower(username) = lower($1)",
    [username],
    function (err, data) {
      if (err) return callback(err);
...
exports.addRecoverId = function (userId, ipAddress, callback) {
  ...
  query(
    "INSERT INTO recovery (id, user_id, ip)  values($1, $2, $3)",
    [recoveryId, userId, ipAddress],
    function (err, res) {
      if (err) return callback(err);
...
exports.getUserBySessionId = function (sessionId, callback) {
  ...
  query(
    "SELECT * FROM users_view WHERE id = (SELECT user_id FROM sessions WHERE id = $1 AND ott = false AND expired > now())",
    [sessionId],
...
exports.getUserByValidRecoverId = function (recoverId, callback) {
  ...
  query(
    "SELECT * FROM users_view WHERE id = (SELECT user_id FROM recovery WHERE id = $1 AND used = false AND expired > NOW())",
    [recoverId],
    function (err, res) {
      if (err) return callback(err);
...
exports.getGame = function (gameId, callback) {
  ...
  query(
    "SELECT * FROM games " +
      ...
      "WHERE games.id = $1 AND games.ended = TRUE",
    [gameId],
    function (err, result) {
      if (err) return callback(err);
...
exports.getGamesPlays = function (gameId, callback) {
  query(
    "SELECT u.username, p.bet, p.cash_out, p.bonus FROM plays p, users u " +
      " WHERE game_id = $1 AND p.user_id = u.id ORDER by p.cash_out/p.bet::float DESC NULLS LAST, p.bet DESC",
    [gameId],
    function (err, result) {
      if (err) return callback(err);
...
exports.getUserPlays = function (userId, limit, offset, callback) {
  ...
  query(
    "SELECT p.bet, p.bonus, p.cash_out, p.created, p.game_id, g.game_crash FROM plays p " +
...
exports.getGiveAwaysAmount = function (userId, callback) {
  assert(userId);
  query(
    "SELECT SUM(g.amount) FROM giveaways g where user_id = $1",
    [userId],
    function (err, result) {
      if (err) return callback(err);
...
exports.addGiveaway = function (userId, callback) {
  ...
  getClient(function (client, callback) {
    client.query(
      "SELECT last_giveaway FROM users_view WHERE id = $1",
      [userId],
      function (err, result) {
        if (err) return callback(err);
        ...
        client.query(
          "INSERT INTO giveaways(user_id, amount) VALUES($1, $2) ",
          [userId, amount],
          function (err) {
            if (err) return callback(err);
        ...
        client.query(
          ...
          function (err, result) {
            ...
            if (result.rows.length === 0)
              return callback(username + " didnt exists");
...
exports.getPublicStats = function (username, callback) {
  var sql =
    "SELECT id AS user_id, username, gross_profit, net_profit, games_played, " +
  ...
  getClient(function (client, callback) {
    client.query(
      ...
      function (err, response) {
        ...
        client.query(
          "INSERT INTO fundings(user_id, amount, bitcoin_withdrawal_address, withdrawal_id) " +
...
exports.getWithdrawals = function (userId, callback) {
  ...
  query(
    "SELECT * FROM fundings WHERE user_id = $1 AND amount < 0 ORDER BY created DESC",
    [userId],
    function (err, result) {
      if (err) return callback(err);
...
exports.getDepositsAmount = function (userId, callback) {
  assert(userId);
  query(
    "SELECT SUM(f.amount) FROM fundings f WHERE user_id = $1 AND amount >= 0",
    [userId],
    function (err, result) {
      if (err) return callback(err);
      callback(null, result.rows[0]);
...
exports.getLeaderBoard = function (byDb, order, callback) {
  var sql =
    "SELECT * FROM leaderboard ORDER BY " + byDb + " " + order + " LIMIT 100";
...
exports.addChatMessage = function (
  userId,
  created,
  message,
  channelName,
  ...
  var sql =
    "INSERT INTO chat_messages (user_id, created, message, channel, is_bot) values($1, $2, $3, $4, $5)";
  query(
    sql,
    [userId, created, message, channelName, isBot],
    function (err, res) {
      if (err) return callback(err);
...
exports.getChatTable = function (limit, channelName, callback) {
  ...
  var sql =
    "SELECT chat_messages.created AS date, 'say' AS type, users.username, users.userclass AS role, chat_messages.message, is_bot AS bot " +
...
exports.getSiteStats = function (callback) {
  ...
  var tasks = [
    ...
    function (callback) {
      query(
        "SELECT COALESCE(SUM(fundings.amount), 0)::bigint sum FROM fundings WHERE amount < 0",
        as("withdrawals", callback)
      );
    },
    function (callback) {
      query(
        "SELECT COUNT(*) FROM games WHERE ended = false AND created < NOW() - interval '5 minutes'",
        as("unterminated_games", callback)
      );
    },
    function (callback) {
      query(
        "SELECT COUNT(*) FROM fundings WHERE amount < 0 AND bitcoin_withdrawal_txid IS NULL",
        as("pending_withdrawals", callback)
      );
    },
    function (callback) {
      query(
        "SELECT COALESCE(SUM(fundings.amount), 0)::bigint sum FROM fundings WHERE amount > 0",
        as("deposits", callback)
      );
    },
    function (callback) {
      query(
