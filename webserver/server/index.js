...
if (config.PRODUCTION) {
  ...
  app.use(
    express.static(path.join(__dirname, "../client_new"), {
      maxAge: twoWeeksInSeconds * 1000,
    })
  );
  app.use(
    "/client_old",
    express.static(path.join(__dirname, "../client_old"), {
      maxAge: twoWeeksInSeconds * 1000,
    })
  );
  app.use(
  ...
  database.getUserBySessionId(sessionId, function (err, user) {
    if (err) {
      ...
      if (err === "NOT_VALID_SESSION") {
        ...
        console.error(
          "[INTERNAL_ERROR] Unable to get user by session id " +
            sessionId +
            ":",
          err
        );
    ...
    user.moderator =
      user.userclass === "admin" || user.userclass === "moderator";
    req.user = user;
    next();
  });
});
...
function errorHandler(err, req, res, next) {
  if (err) {
    ...
    console.warning(
      "A 'next()' call was made without arguments, if this an error or a msg to the client?"
    );
  }
}
...
io.use(function (socket, next) {
  ...
  var sessionId = socket.request.headers.cookie
    ? socket.request.headers.cookie.id
    : null;
  ...
  database.getUserBySessionId(sessionId, function (err, user) {
    ...
    if (err) {
      if (err === "NOT_VALID_SESSION") {
        ...
        console.error(
          "[INTERNAL_ERROR] Unable to get user in socket by session " +
            sessionId +
            ":",
          err
        );
        next(
          new Error(
            "Unable to get the session on the server, logged as a guest."
          )
        );
