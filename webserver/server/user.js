...
exports.login = function (req, res, next) {
  ...
  database.validateUser(username, password, otp, function (err, userId) {
    if (err) {
      ...
      return next(
        new Error("Unable to validate user " + username + ": \n" + err)
      );
    }
    assert(userId);
    ...
    database.createSession(
      ...
      userAgent,
      remember,
      function (err, sessionId, expires) {
        if (err)
          return next(
            new Error(
              "Unable to create session for userid " + userId + ":\n" + err
            )
          );
...
exports.logout = function (req, res, next) {
  ...
  database.expireSessionsByUserId(userId, function (err) {
    if (err) return next(new Error("Unable to logout got error: \n" + err));
    res.redirect("/");
  });
};
...
exports.profile = function (req, res, next) {
  ...
  var page = null;
  if (req.query.p) {
    //The page requested or last
    page = parseInt(req.query.p);
    if (!Number.isFinite(page) || page < 0) return next("Invalid page");
  }

  if (!username) return next("No username in profile");

  database.getPublicStats(username, function (err, stats) {
    if (err) {
      if (err === "USER_DOES_NOT_EXIST") return next("User does not exist");
      else return next(new Error("Cant get public stats: \n" + err));
    }
    ...
    var firstPageResultCount =
      stats.games_played - (pages - 1) * resultsPerPage;

    var showing = page ? resultsPerPage : firstPageResultCount;
    var offset = page
      ? firstPageResultCount + (pages - page - 1) * resultsPerPage
      : 0;
    ...
    async.parallel(tasks, function (err, results) {
      ...
      if (!lib.isInt(netProfitOffset))
        return next(
          new Error(
            "Internal profit calc error: " +
              username +
              " does not have an integer net profit offset"
          )
        );
      ...
      var previousPage;
      if (pages > 1) {
        if (page && page >= 2) previousPage = "?p=" + (page - 1);
        else if (!page) previousPage = "?p=" + (pages - 1);
      }

      var nextPage;
      if (pages > 1) {
        if (page && page < pages - 1) nextPage = "?p=" + (page + 1);
        else if (page && page == pages - 1) nextPage = stats.username;
      }
      ...
      if (err.message === "NOT_ELIGIBLE") {
        return res.render("request", {
          ...
          warning:
            "You have to wait " +
            err.time +
            " minutes for your next give away.",
        });
...
exports.account = function (req, res, next) {
  ...
  async.parallel(tasks, function (err, ret) {
    if (err) return next(new Error("Unable to get account info: \n" + err));
...
exports.resetPassword = function (req, res, next) {
  ...
  if (!password)
    return res.redirect("/security?err=Enter%20your%20old%20password");

  var notValid = lib.isInvalidPassword(newPassword);
  if (notValid)
    return res.redirect(
      "/security?err=new%20password%20not%20valid:" + notValid
    );

  if (newPassword !== confirm)
    return res.redirect(
      "/security?err=new%20password%20and%20confirmation%20should%20be%20the%20same."
    );

  database.validateUser(user.username, password, otp, function (err, userId) {
    if (err) {
      if (err === "WRONG_PASSWORD")
        return res.redirect("/security?err=wrong password.");
      if (err === "INVALID_OTP")
        return res.redirect("/security?err=invalid one-time password.");
    ...
    database.changeUserPassword(user.id, newPassword, function (err) {
      ...
      database.expireSessionsByUserId(user.id, function (err) {
        if (err)
          return next(
            new Error(
              "Unable to delete user sessions for userId: " +
                user.id +
                ": \n" +
                err
            )
...
exports.editEmail = function (req, res, next) {
  ...
  if (email.length === 0) {
    ...
    if (notValid)
      return res.redirect("/security?err=email invalid because: " + notValid);
  }

  notValid = lib.isInvalidPassword(password);
  if (notValid)
    return res.render("/security?err=password not valid because: " + notValid);
...
exports.security = function (req, res) {
  ...
  if (!user.mfa_secret) {
    ...
    var qrUri =
      "otpauth://totp/bustabit:" +
      user.username +
      "?secret=" +
      user.mfa_potential_secret +
      "&issuer=bustabit";
...
exports.disableMfa = function (req, res, next) {
  ...
  if (!secret)
    return res.redirect("/security?err=Did%20not%20sent%20mfa%20secret");
  if (!user.mfa_secret)
    return res.redirect("/security?err=2FA%20is%20not%20enabled");
...
exports.sendPasswordRecover = function (req, res, next) {
  ...
  database.getUsersFromEmail(email, function (err, users) {
    if (err) {
      if (err === "NO_USERS") return res.render("forgot-password", messageSent);
      else
        return next(
          new Error("Unable to get users by email " + email + ": \n" + err)
        );
    }
    ...
    async.each(
      ...
      function (user, callback) {
        database.addRecoverId(
          user.id,
          remoteIpAddress,
          function (err, recoveryId) {
      ...
      function (err) {
        if (err) return next(new Error("Unable to add recovery id :\n" + err));
...
exports.validateResetPassword = function (req, res, next) {
  ...
  database.getUserByValidRecoverId(recoverId, function (err, user) {
    if (err) {
      ...
      return next(
        new Error("Unable to get user by recover id " + recoverId + "\n" + err)
      );
    }
...
exports.resetPasswordRecovery = function (req, res, next) {
  ...
  database.changePasswordFromRecoverId(
    recoverId,
    password,
    function (err, user) {
      if (err) {
        ...
        return next(
          new Error(
            "Unable to change password for recoverId " +
              recoverId +
              ", password: " +
              password +
              "\n" +
      ...
      database.createSession(
        ...
        function (err, sessionId) {
          if (err)
            return next(
              new Error(
                "Unable to create session for password from recover id: \n" +
                  err
              )
            );
...
exports.handleWithdrawRequest = function (req, res, next) {
  ...
  try {
    ...
    if (
      version !== bitcoinjs.networks.bitcoin.pubKeyHash &&
  ...
  database.validateUser(user.username, password, otp, function (err) {
    if (err) {
      ...
      //Should be an user
      return next(
        new Error("Unable to validate user handling withdrawal: \n" + err)
      );
    }
      ...
      if (err) {
        if (err === "NOT_ENOUGH_MONEY")
          ...
          return res.render("withdraw-request", {
            ...
            id: uuid.v4(),
            warning:
              "Please reload your page, it looks like you tried to make the same transaction twice.",
          });
          ...
          return res.render("withdraw-request", {
            ...
            id: uuid.v4(),
            success:
              "Your transaction is being processed come back later to see the status.",
          });
