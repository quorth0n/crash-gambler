...
function contact(origin) {
  ...
  return function (req, res, next) {
    ...
    if (!from)
      return res.render(origin, { user: user, warning: "email required" });

    if (!message)
      return res.render(origin, { user: user, warning: "message required" });
    ...
    sendEmail.contact(from, message, null, function (err) {
      ...
      return res.render(origin, {
        user: user,
        success:
          "Thank you for writing, one of my humans will write you back very soon :) ",
      });
    });
  };
}
...
function restrict(req, res, next) {
  if (!req.user) {
    res.status(401);
    if (req.header("Accept") === "text/plain") res.send("Not authorized");
    else res.render("401");
    return;
  } else next();
}
...
function adminRestrict(req, res, next) {
  if (!req.user || !req.user.admin) {
    res.status(401);
    if (req.header("Accept") === "text/plain") res.send("Not authorized");
    else res.render("401"); //Not authorized page.
    return;
  }
  next();
}
...
function recaptchaRestrict(req, res, next) {
  ...
  recaptchaValidator.callback(
    config.RECAPTCHA_PRIV_KEY,
    recaptcha,
    req.ip,
    function (err) {
      if (err) {
        if (typeof err === "string")
          res.send(
            "Got recaptcha error: " + err + " please go back and try again"
          );
        else {
...
function tableDev() {
  return function (req, res) {
    if (config.PRODUCTION) return res.status(401);
...
function requestDevOtt(id, callback) {
  ...
  var options = {
    url: "https://www.retrocrash.com/ott",
    include: true,
    method: "POST",
    cookie: "id=" + id,
  };
...
module.exports = function (app) {
  ...
  app.get("/error", function (req, res, next) {
    // Sometimes we redirect people to /error
