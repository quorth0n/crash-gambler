...
module.exports = function (server, game, chat) {
  ...
  function joined(socket, loggedIn) {
    ...
    console.log(
      ...
      clientCount,
      " - ",
      loggedIn ? loggedIn.username : "~guest~"
    );
    ...
    socket.on("cash_out", function (ack) {
      ...
      game.cashOut(loggedIn, function (err) {
        if (err) {
          ...
          else return console.log("[INTERNAL_ERROR] unable to cash out: ", err); // TODO: should we notify the user?
        }
        ...
        switch (cmd) {
          ...
          case "shadowmute":
            if (loggedIn.moderator) {
              ...
              chat.mute(shadow, loggedIn, username, timespec, function (err) {
          ...
          case "unmute":
            if (loggedIn.moderator) {
              ...
              chat.unmute(loggedIn, username, function (err) {
                if (err) return sendErrorChat(socket, err);
              });
            }
            break;
